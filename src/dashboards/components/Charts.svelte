<script lang="ts">
  ...
  const renderAdminCharts = () => {
    adminLocation = [$admin.adm4_name, $admin.adm3_name, $admin.adm2_name, $admin.adm1_name, $admin.adm0_name]
      .filter(Boolean)
      .join(', ')
    adminBarValues = []
    for (let i = 2020; i >= 2012; i--) {
      adminBarValues = [...adminBarValues, { year: i, value: $admin[`hrea_${i}`], category: HREA_ID }]
    }
  }
