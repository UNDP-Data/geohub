...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const mosaicJsonPath = await createTitilerMosaicJsonEndpoint(searchResult.urls, searchResult.filter)
  // create mosaicjson fetch API.
  // this code will not work in local computer.
  // change to https://dev.undpgeohub.org when you develop in local machine.
  const mosaicJsonUrl = `${url.origin}/stac/mosaicjson?file=${mosaicJsonPath}`
...
const searchStacItemUrls = async (url: string, bbox: number[], targetAsset: string) => {
  ...
  fc.features.forEach((f) => {
    const assetUrl = `${f.assets[targetAsset].href}?${sasToken}`
    const b64EncodedUrl: string = getBase64EncodedUrl(assetUrl)
    itemUrls.push(b64EncodedUrl)
  })
  return {
    urls: itemUrls,
...
const createTitilerMosaicJsonEndpoint = async (urls: string[], filter: string) => {
  ...
  const blobPath = await storeMosaicJson2Blob(json, filter)
  console.log(`mosaicjson was generated at: ${blobPath}`)
  return blobPath
}
