<script lang="ts">
  ...
  const propertyName = 'heatmap-radius'
  const style = $map.getStyle().layers.filter((layer: LayerSpecification) => layer.id === layerId)[0]

  let values = [style.paint && style.paint[propertyName] ? style.paint[propertyName] : 30.0]
  $: values, setLineWidth()
...
{#if style.type === LayerTypes.HEATMAP}
  <StyleControlGroup title="Heatmap Radius">
    <div class="slider">
      <RangeSlider bind:values float min={0} max={100} step={1} pips rest={false} />
    </div>
  </StyleControlGroup>
{/if}
