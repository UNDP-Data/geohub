...
import { recolorPngToSvg } from './helpers';
...
`;
	 ...
	 * @returns SVG string
	 */
	public getCategorizedLegend = async (
		colors: [number, number, number, number][] | string[],
		values: number[][],
		options?: SvgLegendCreatorOptions
	) => {
		const colorContent: string[] = [];
		for (let index = 0; index < colors.length; index++) {
			...
			if (options?.shape) {
				shape = options.shape;
				if (options.shape.startsWith('data:image')) {
					shape = await recolorPngToSvg(options.shape, color);
				}
				const size = options.shape.indexOf('<circle') === -1 ? '20' : '10';
				shape = shape.replace(/{size}/g, size).replace(/{color}/g, color);
			}
	 ...
	 * @returns SVG string
	 */
	public getUniqueValueLegend = async (
		colors: [number, number, number, number][] | string[],
		values: string[],
		options?: SvgLegendCreatorOptions
	) => {
