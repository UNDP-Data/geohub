<script lang="ts">
	...
	const getValue = () => {
		...
		value = convertFunctionToExpression(value, defaultLineWidth) as number;
		return value;
	};
	...
	onMount(() => {
		...
		console.log($numberOfClassesStore);
	});
	...
	const updateMapFromRows = () => {
		if (propertySelectValue.length === 0) {
			let v = Array.isArray(value) ? defaultLineWidth : value;
					...
					{#each colorMapRows as row, index}
						<tr data-testid="line-width-row-container">
							<td class="has-text-centered">
								<div
									style={`margin-top: 5px; width: 100px; height: ${row.value}px; background-color: ${defaultColor};`}
								/>
							</td>
							<td>
								<NumberInput
